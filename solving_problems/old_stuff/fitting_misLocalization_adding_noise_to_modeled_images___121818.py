...
#######################################################################
## Optics stuff.  
sensor_size = parameters['optics']['sensor_size']
...
def quiver_plot(x_plot, y_plot, angles, plot_limits=[-25,550],
               ...
               nanorod_angle=0):
    if true_mol_angle is None: true_mol_angle = angles
...
class CoupledDipoles:
    ...
    def dipole_fields(self, locations, mol_angle=0, plas_angle=np.pi/2):
        ...
        p0, p1 = cp.dipole_mags_gened(mol_angle, plas_angle, d_col=d, E_d_angle=None,
                                      alpha0_diag=alpha0_diag_dyad,
                                      alpha1_diag=alpha1_diag_dyad)
        mol_E = self.mb_p_fields(dipole_mag_array=p0, dipole_coordinate_array=d) 
        plas_E = self.mb_p_fields(dipole_mag_array=p1, dipole_coordinate_array=np.zeros(d.shape))
    ...
    #     p0_unc_E = self.mb_p_fields(dipole_mag_array=p0_unc[None,:], dipole_coordinate_array=np.zeros(d[0][None,:].shape))
        if type(mol_angle)==np.ndarray and mol_angle.shape[0]>1:
            p0_unc_E = self.mb_p_fields(dipole_mag_array=p0_unc, dipole_coordinate_array=d)
        elif (type(mol_angle) == int or
              type(mol_angle) == float or
              type(mol_angle) == np.float64 or
              (type(mol_angle) == np.ndarray and mol_angle.shape[0]==1)
              ):
            p0_unc_E = self.mb_p_fields(dipole_mag_array=p0_unc[None,:], dipole_coordinate_array=d)
...
class FitModelToData:
    ...
    def fit_model_to_image_data(self, images=None):
        ...
        for i in np.arange(num_of_images):
            ...
            print('initial guess for molecule {} location: ({},{})'.format(i, ini_x, ini_y))
#             ini_x = -50
#             ini_y = 100
#             print(ini_x, ini_y)
    ...
    def raveled_model_of_params(self, fit_params):
        ...
        exp_instance = MolCoupNanoRodExp(locations, 
                                         ...
                                         x_obv_grid=self.x_obv_grid, 
                                         y_obv_grid=self.y_obv_grid)
...
#                    color='white',pivot='middle')
    ...
    def plot_raveled_image(self, image, ax=None):
        if ax is None:
            plt.figure(figsize=(3,3),dpi=600)
            plt.pcolor(self.x_obv_grid/nm,self.y_obv_grid/nm,image.reshape(self.x_obv_grid.shape))
            plt.colorbar()
        else:
            ax.contour(self.x_obv_grid/nm,self.y_obv_grid/nm,image.reshape(self.x_obv_grid.shape),
...
class FitModelToNoisedModel(FitModelToData):
    ...
    def make_image_noisy(self, image, PEAK=1):
        ...
        noised_normded_image_data = np.random.poisson(normed_image/255.0* PEAK) / (PEAK) *255
    ...
    def fit_model_to_noised_model_image_data(self, PEAK=5000):
        ...
        self.model_fit_results = self.fit_model_to_image_data(images=self.noised_image_data)
        return self.model_fit_results
    
#     def plot_fit_results(self):
    def plot_fit_results_as_quiver_map(self, fitted_exp_instance, plot_limits=None):
        '''...'''
        ...
        quiv_ax, = quiver_plot(fitted_exp_instance.mol_locations[:,0], 
        ...
        scatter_centroids_wLine(fitted_exp_instance.mol_locations[:,0], 
                                fitted_exp_instance.mol_locations[:,1],
...
if __name__ == '__main__':
    # ### want to build quiver plot for fits 
    ...
    test_fit_inst_closer_random_noi = FitModelToNoisedModel(test_instance3.trial_images)
    closer_fit_results_random_ori_noi = test_fit_inst_closer_random_noi.fit_model_to_noised_model_image_data()

    test_fit_inst_closer_random_noi.plot_fit_results_as_quiver_map(test_instance3)

    test_fit_inst_closer_random_noi.plot_raveled_image(test_fit_inst_closer_random_noi.noised_image_data[4])
    test_fit_inst_closer_random_noi.plot_image_from_params(closer_fit_results_random_ori_noi[4])


    # In[91]:

    for i in range(test_fit_inst_closer_random_noi.model_fit_results.shape[0]):
        test_fit_inst_closer_random_noi.plot_contour_fit_over_data(i)
    ...
    # ### making things a little more noisy

    # In[92]:

    test_fit_inst_closer_random_noi = FitModelToNoisedModel(test_instance3.trial_images)
    closer_fit_results_random_ori_noi = test_fit_inst_closer_random_noi.fit_model_to_noised_model_image_data(PEAK=2000)
    ...
    test_fit_inst_closer_random_noi = FitModelToNoisedModel(test_instance3.trial_images)
    closer_fit_results_random_ori_noi = test_fit_inst_closer_random_noi.fit_model_to_noised_model_image_data(PEAK=500)
    ...
    locations_4, angles_4 = random_ori_mol_placement(x_min=0, x_max=250, y_min=0, y_max=250, mol_grid_pts_1D = 3)
    test_instance4 = MolCoupNanoRodExp(locations_4, angles_4)

    for peak_val in [5000, 2000, 500]:

        print('\n\n\n\n\n\n\n PEAK={}'.format(peak_val))
        test_fit_inst_closer_random_noi = FitModelToNoisedModel(test_instance4.trial_images)
        closer_fit_results_random_ori_noi = test_fit_inst_closer_random_noi.fit_model_to_noised_model_image_data(PEAK=peak_val)

        test_fit_inst_closer_random_noi.plot_fit_results_as_quiver_map(test_instance4)
