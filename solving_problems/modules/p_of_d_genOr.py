...
def unit_vector_from_rows(row_vecs): 
...
############################################################################
def pp_coupling_constant(
    drive_hbar_w, p1_hat, p2_hat, d_col,
    ):
    ...
    n_hat = unit_vector_from_rows(d_col) ## returns shape = (N,3)
    d = row_mag(d_col) ## returns shape = (N,1)
    # print('d.shape')
    ...
    nf_if_vecs = (
        3.*np.multiply(
            np.sum(np.multiply(n_hat, p1_hat),axis=-1), 
            np.sum(np.multiply(n_hat, p2_hat),axis=-1)
            ) ## yields shape = (N,)
        - np.sum(np.multiply(p1_hat, p2_hat),axis=-1) ## also yields shape = (N,)
        ) ## yields shape = (N,)
    print('old n_hat = ',n_hat)
    print('old nf_if_vecs = ',nf_if_vecs)
...
def g_l1(mol_orientation, plas_orientation, drive_hbar_w, d_col):
    
    gp = pp_coupling_constant(
        drive_hbar_w, 
        p1_hat=plas_orientation, 
        p2_hat=mol_orientation, 
        d_col=d_col,
        ######    
    ...
    # print('g(d = {}, p_hat = {}) = {}'.format(d_col,mol_orientation,gp))
    return gp
