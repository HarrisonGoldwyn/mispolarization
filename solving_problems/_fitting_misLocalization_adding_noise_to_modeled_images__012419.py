...
01/24/18
    Removed dependence on parameters file, all calls to variables that used to
    be define there are now moved to kwargs inside functions.

    Saved with leading underscore to indicate that this ^ update is not yet 
    finished.inc_int
...
# height = 2*mm  # a
lso defines objective lens focal length 
...
class DipoleProperties(object):
    """
    Initializes dipole properties from driving frequency and fit 
    parameters from independant molecula and nanoparticle. 

    Args:
        hbar_omega: driving frequency ascribing harmonic time dependance for 
            all dynamics
        drude_fit_params: dictionary containing the following material properties of the
            metal, {
                eps_inf:
                hbar_omega_p:
                hbar*gamma_nr:
        ...
        eps_b: dielectric function of the background medium surrounding the
            metal
        fluo_params: {
            ext_coeff:
            emission_hbar_gamma:
    ...
    def __init__(
        ...
        self.metal_params = (
            ## eps_inf, hbar*omega_p, hbar*gamma_nr,  
            ## (eps_b: not used as fit param), a_x, a_yz
            drude_fit_params['eps_inf'], 
            drude_fit_params['hbar_omega_p']/hbar, 
            hbar_gamma_nr/hbar, 
            eps_b, 
            drude_fit_params['a_x']*nm, 
            drude_fit_params['a_yz']*nm
            )

        self.alpha0_diag_dyad = cp.sparse_polarizability_tensor(
            mass=cp.fluorophore_mass(
                ext_coef=fluo_params['ext_coeff'], 
                gamma=fluo_params['emission_hbar_gamma']/hbar
                ), 
            w_res=self.hbar_omega/hbar, 
            w=self.hbar_omega/hbar, 
            gamma_nr=fluo_params['emission_hbar_gamma']/hbar,
            a=0, 
            eps_inf=1, 
            ebs_b=1
            )
...
class BeamSplitter(object):
    """ Operations a la Curly for splitting the field into orthogonal 
    components and calculating average powers form each.

    Args:
        inc_int: incident intensity to 
        """
    def __init__(self, inc_int):
        pass
...
class CoupledDipoles(DipoleProperties, PlottingStuff, FittingTools):
    ...
    def __init__(self, hbar_omega, obs_points=None, background_ref_index=None):
        """
        ...
        self.hbar_omega = hbar_omega 
        if background_ref_index == None:
            self.background_ref_index = 1
        else: 
            self.background_ref_index = background_ref_index
    ...
    def mb_p_fields(
        ...
        px_fields = np.asarray(
            afi.E_field(
                0, v_rel_obs_x_pts, v_rel_obs_y_pts, k,
                )
            )
        py_fields = np.asarray(
            afi.E_field(
                np.pi/2, v_rel_obs_x_pts, v_rel_obs_y_pts, k,
                )
            )
