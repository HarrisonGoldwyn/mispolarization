...
# sensor_size = 1
# _height = 300*nm
resolution = 50  # grid _resolution
...
########## I just want to plot the magnitude of the electric field
        ...
        e_field_c = y_di_E_field
        e_field = np.real(e_field_c)

        b_field_c = y_di_B_field
        ...
        diff_ini_gue = (1, 43*nm, 0.0, 1e1, 1e1, 0.0, 0.0)
        try:
            ff_popt, ff_pcov = opt.curve_fit(
                twoD_Gaussian,
                eye[1]/(nm * magnification), 
                eye[2]/(nm * magnification),
                normed_data,
            ...
            print(
                'fit params for diffraction; \n {}'.format(ff_popt)
                )

            diff_data_fitted = twoD_Gaussian((X_normed, Y_normed), *ff_popt)
            print('diff fields at sep= {0}, fit'.format(seps[i]))

            diff_cont_plot = diff_ax.contour(
                eye[1]/(nm * magnification), 
                eye[2]/(nm * magnification), 
                diff_data_fitted.reshape(np.shape(eye[1])), 
                5, 
                colors='w'
                )

        except RuntimeError: 
            diff_data_fitted = None
            print('diff fields at sep= {0}, NOT fit'.format(seps[i]))
