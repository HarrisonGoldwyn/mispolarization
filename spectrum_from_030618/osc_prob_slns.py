''' 
Fluo energy fixed, drive energy varies by param.yaml
'''
...
def seperations(min_sep, max_sep, step_size):
    return np.arange(min_sep, max_sep, step_size)

def seperation_columb(min_sep, max_sep, step_size):
    return np.arange(min_sep, max_sep, step_size)[:,None]


############################################################################
### Functions for generating coupling constants for equations of motion  ###
############################################################################
def pp_coupling_constant(drive_hbar_w, d1_or, d2_or, seperation_col ):
    ''' returns columb corresponding to seperations '''
    d1_pos = np.array([-1,0,0]) * (seperation_col/2.)
    d2_pos = np.array([1,0,0]) * (seperation_col/2.)
...
def fluo_osci_mag_phase(fluo_mass, fluo_res,
    ...
    return [real_dipole_amplitude, phase]
...
if parameters['plasmon']['incorperate_parameters_from_fit'] == True:
    ...
    plas_l1_mass= parameters['plasmon']['fit_mass'] * 1e-32  
...
plas_l2_mass= parameters['plasmon']['fit_l2_mass'] * 1e-22
...
fluo_non_rad_damp_ene = parameters['fluorophore']['lw_gamma']
fluo_ress_ene = parameters['fluorophore']['res_energy']
## ^ never called since fluo is always driven at resonance
...
def g_l1(orientation, drive_hbar_w):
    gp = pp_coupling_constant(
        drive_hbar_w,
        d1_or=orientation,
        d2_or=orientation,
        seperation_col=seperation_columb(
...
def sum_gSqrd_on_mD_terms(orientation, drive_hbar_w):
    gs_on_mds = (
        g_l1(orientation, drive_hbar_w)**2.
        /
...
def plas_l1_osc(orientation, drive_hbar_w): 
    '''returns amp and phase vectors
    '''
    plas_l1_amp, plas_l1_phase = plas_osci_mag_phase(
        g_l1=g_l1(orientation, drive_hbar_w),
        ...
        sum_gSqrd_on_mD=sum_gSqrd_on_mD_terms(orientation, drive_hbar_w),
    ...
    return [plas_l1_amp, plas_l1_phase]

if parameters['plasmon']['include_quadrupole']==True:
    def plas_l2_osc(orientation, drive_hbar_w): 
        '''returns amp and phase vectors
        '''
        plas_l2_amp, plas_l2_phase = plas_osci_mag_phase(
            g_l1=g_l2(drive_hbar_w),
        ...
        return [plas_l2_amp, plas_l2_phase]

def fluo_osc(orientation, drive_hbar_w):
    '''
    '''
    fluo_amp, fluo_phase = fluo_osci_mag_phase(
        fluo_mass=fluo_mass, 
        fluo_res=eV_to_Hz(fluo_ress_ene),
        fluo_damp=eV_to_Hz(fluo_non_rad_damp_ene), 
        drive_hbar_w=drive_hbar_w, 
